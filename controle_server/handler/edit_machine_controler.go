...
func Edit_machine_controler(w http.ResponseWriter, r *http.Request) {
	...
	// build parameters
	params := []interface{}{}
	if payload.ZONE				!= "" { params = append(params, payload.ZONE) }
	if payload.NAME 			!= "" { params = append(params, payload.NAME) }
	if payload.MANUAL			!= "" { params = append(params, payload.MANUAL) }
	if payload.PRICE_BOOKING	!= "" { params = append(params, payload.PRICE_BOOKING) }
	if payload.PRICE_USAGE		!= "" { params = append(params, payload.PRICE_USAGE) }
