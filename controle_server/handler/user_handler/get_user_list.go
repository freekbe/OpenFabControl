...
func Get_user_list(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.Respond_error(w, "Internal server error1", http.StatusInternalServerError)
		return
	}
	defer rows.Close()
	...
	if err := json.NewEncoder(w).Encode(users); err != nil {
		utils.Respond_error(w, "Internal server error3", http.StatusInternalServerError)
		return
	}
}
